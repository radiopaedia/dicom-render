...
// Use a virtual canvas in non-browser contexts
if ('CanvasRenderingContext2D' in globalThis === false) {
  // `canvas` is not bundled so we can simply import it async when needed
...
export async function renderImage(dicomImage, element) {
  ...
  const renderedImageDataPromise = new Promise(resolve => {
    const callback = event => {
      // The shim doesn't filter events so we need to check for the correct event type
      if (event.type === IMAGE_RENDERED) {
        // This will be document._canvas (the rendercanvas) when running inside the shim
    ...
    if ('addEventListener' in globalThis.document) {
      cornerstoneElement.addEventListener(IMAGE_RENDERED, callback);
    } else {
      // TODO: maybe just implement enough addEventListener on the shim to make it transparent?
      globalThis.document?._listen(callback);
    }
  });
